...
%% Concat precip for full dataset - incomplete
%{
% initialize prcp_ann to store 1 year data
prcp_ann=[];
year_range=[1951:1:2021]; % set number of years here

for i=1:length(year_range); 
    yr = year_range(i);
    oy=num2str(yr);

    for mon=1:2; % set number of months here 
        ...
        %prcp_ann=cat(3,prcp_ann,prcp);
        %prcp_anncat(# of dimensions, prcp_ann, prcp data)
