    ...
    for iday=1:nday
        ...
        for imask=1:nmask % for length of masked grid area
            ...
            if (i==1)
                ...
                %P_results_daily(imask, iday + nday) = sum(p(indx), 'omitnan');
                P_results_daily(imask, (i-1)*30 + iday)= sum(p(indx), 'omitnan');
            end 
        end 
    end
end 

save('/Users/kboothomefolder/Library/CloudStorage/OneDrive-UNC-Wilmington/x_consult/precip/P_results_daily_1951.mat', 'P_results_daily');
